...
| Prop                    | Remarks                                                                                                                                                                                                         |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| renderers               | Each renderer function should be converted to a React Component. See [Custom Rendering guide](./docs/guides/custom-renderers) and the below section, [Migrating Custom Renderers](#migrating-custom-renderers). |
| renderersProps          | To access those props from a custom renderer, you should now use `useRendererProps` hook.                                                                                                                       |
| source                  | A new type of source is available, `source.dom`. See [DOM Tampering](./docs/guides/dom-tampering#prerendering) for a use case.                                                                                  |
...
| tagsStyles              | Those styles are now mixed styles records, which are unlikely to break. See [Styling Components](./docs/guides/styling).                                                                                        |
| classesStyles           | Those styles are now mixed styles records, which are unlikely to break. See [Styling Components](./docs/guides/styling).                                                                                        |
...
| onParsed                       | onTTreeUpdate                                     | The structure has changed. See [Transient Render Engine](./docs/flow/transient-render-engine).                     |
| listsPrefixesRenderers         | customListStyleSpecs                              | The API has changed, see [Lists, customization](./docs/content/lists#defining-and-customizing-markers-pseudo-elements). |
...
| alterData            | You can use `domVisitors.onText` instead. See [Dom Tampering](./docs/guides/dom-tampering#example-altering-data).                                               |
| alterChildren        | You can use `domVisitors.onElement` instead. See [Dom Tampering](./docs/guides/dom-tampering#example-inserting-elements).                                       |
| alterNode            | You can use `domVisitors.onElement` instead. See [Dom Tampering](./docs/guides/dom-tampering#example-removing-elements).                                        |
| allowWhitespaceNodes | White-space collapsing is now fully supported. See [Transient Render Engine, whitespace collapsing](./docs/flow/transient-render-engine#whitespace-collapsing). |
...
| ------------------------------------------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `htmlAttribs`                               | `tnode.attributes`                            | The TNode is the intermediary representation of the element to render. See [Architecture](./docs/architecture).                                                                                                                                                                                  |
| `children`                                  | Alternative exists.                           | You can still render children. However, instead of having access to the children as React elements, you can inspect `tnode.children` and render these with `TNodeChildrenRenderer`. This is much more flexible than the legacy API. See [Custom Rendering](./docs/guides/custom-renderers). |
...
See [Custom Rendering, registering a new tag](./docs/guides/custom-renderers#example-registering-a-new-tag).
