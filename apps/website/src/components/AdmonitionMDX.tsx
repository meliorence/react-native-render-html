...
const infimaMappings: Record<AdmonitionType, string> = {
  note: 'secondary',
  important: 'info',
  tip: 'success',
  warning: 'danger',
...
// From https://github.com/elviswolcott/remark-admonitions/blob/master/lib/index.js
const types = {
  note: {
    ifmClass: 'secondary',
    keyword: 'note',
...
export default function AdmonitionMDX({
  type,
  children
}: PropsWithChildren<{
  type: AdmonitionType;
  ...
  return (
    <div
      className={`admonition admonition-${type} alert alert--${infimaMappings[type]}`}>
      <div className="admonition-heading">
        <h5>
          <span
            className="admonition-icon"
            dangerouslySetInnerHTML={{ __html: types[type].svg }}
          />
          {type}
        </h5>
      </div>
